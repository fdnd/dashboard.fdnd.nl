<script>
  let { latestDeployAt } = $props();

  function formatLastUpdated(isoString) {
    if (!isoString) return null;

    const date = new Date(isoString);
    const now = new Date();

    const startOfDay = (d) => new Date(d.getFullYear(), d.getMonth(), d.getDate());

    const dDay = startOfDay(date).getTime();
    const nDay = startOfDay(now).getTime();
    const diffDays = Math.round((nDay - dDay) / (1000 * 60 * 60 * 24));

    const timePart = date.toLocaleTimeString('nl-NL', {
      hour: '2-digit',
      minute: '2-digit'
    });

    if (diffDays === 0) return `today at ${timePart}`;
    if (diffDays === 1) return `yesterday at ${timePart}`;

    return date.toLocaleString('nl-NL', {
      dateStyle: 'short',
      timeStyle: 'short'
    });
  }

  let lastUpdated = $derived(formatLastUpdated(latestDeployAt));
</script>

<p>
  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-clock-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M21 12a9 9 0 1 0 -9.972 8.948c.32 .034 .644 .052 .972 .052" /><path d="M12 7v5l2 2" /><path d="M18.42 15.61a2.1 2.1 0 0 1 2.97 2.97l-3.39 3.42h-3v-3l3.42 -3.39" /></svg>
  <small>Updated 
  {#if lastUpdated}
    {lastUpdated}
  {:else}
    unknown
  {/if}
  </small>
</p>

<style>
  p {
    --_position-right: var(--padding-side);
    position:absolute;
    right:1rem;
    margin-top: .25rem;
    display:flex;
    align-items:center;
    gap:.25rem;

    small {
      font-size: .9rem;
    }

    @media (min-width:30rem) {
      --_position-right: calc(var(--padding-side) * 2);
      right:2rem;
    }
  }
</style>